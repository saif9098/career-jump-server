...
export const getMatchedJobsController = async (req, res) => {
  try {
    const userId = req.user._id;
    ...
    const matchedJobs = jobs.map(job => {
      const profileSkills = profile.skillsArr.map(s => s.toLowerCase());
      const jobSkills = job.requiredSkills.toLowerCase().split(',').map(s => s.trim());
    
      const matchedSkills = profileSkills.filter(skill => jobSkills.includes(skill));
      const skillScore = (matchedSkills.length / jobSkills.length) * 60; // Weighted 60%
    
      const courseStreamMatch = job.eligibility.toLowerCase().includes(profile.course.toLowerCase()) ||
                                job.eligibility.toLowerCase().includes(profile.stream.toLowerCase());
      const courseScore = courseStreamMatch ? 25 : 0;
    
      const expMatch = job.eligibility.toLowerCase().includes(profile.experience.toLowerCase());
      const expScore = expMatch ? 15 : 0;
    
      const matchPercentage = Math.round(skillScore + courseScore + expScore);
